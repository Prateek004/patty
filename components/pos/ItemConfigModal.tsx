...
export default function ItemConfigModal({ item, onClose }: Props) {
  ...
  // All hooks declared unconditionally at top level — fixes Rules of Hooks violation
  ...
  // Reset state whenever the item changes (new item opened)
  useEffect(() => {
    if (!item) return;
    setQty(1);
  ...
  // Derived values (safe even when item is null — just won't display)
      ...
      {item && (
        <div className="px-5 pb-6 space-y-4 pt-1">
                ...
                {item.sizes.map((s) => (
                  ...
                  >
                    ...
                    {s.pricePaise !== item.pricePaise && (
                      <span className="ml-1 text-xs opacity-60">{fmtRupee(s.pricePaise)}</span>
                    )}
                  </button>
                ))}
              </div>
            ...
            <div>
              ...
              <div className="flex gap-2 flex-wrap">
                {item.portions.map((p) => (
                  ...
                  >
                    {p.label}
                    <span className="ml-1 text-xs opacity-60">{fmtRupee(p.pricePaise)}</span>
                  </button>
                ))}
              </div>
            </div>
                ...
                {item.addOns.map((ao) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full flex items-center justify-between px-4 py-3 rounded-xl border-2 transition-all press ${
                        on ? "border-primary-500 bg-primary-50" : "border-gray-100"
                      }`}
                    >
                      <span className={`text-sm font-semibold ${on ? "text-primary-600" : "text-gray-700"}`}>{ao.name}</span>
                      <div className="flex items-center gap-3">
                        {ao.pricePaise > 0 && (
                          <span className="text-sm text-gray-500">+{fmtRupee(ao.pricePaise)}</span>
                        )}
                        <div className={`w-5 h-5 rounded border-2 flex items-center justify-center transition-colors ${on ? "border-primary-500 bg-primary-500" : "border-gray-300"}`}>
          ...
          <div className="flex items-center gap-3 pt-1">
            ...
            <button
              onClick={handleAdd}
              className="flex-1 h-12 bg-primary-500 text-white rounded-2xl font-bold flex items-center justify-between px-5 press shadow-md shadow-primary-100"
            >
